<p align="center">
  <img width="900" alt="data-banner" src="https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExM2ZxN3pyNDF1b2x2cHhpczloYXZ2Z2p1cnN1OGV6a2o2dGd6eTI2NyZlcD12MV9naWZzX3NlYXJjaCZjdD1n/qgQUggAC3Pfv687qPC/giphy.gif" />
</p><h1>Data Analysis Team Members</h1><p align="center"><img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=22&duration=3000&color=007ACC&center=true&vCenter=true&width=700&lines=Transforming+raw+datasets+into+impactful+insights;From+Excel+to+Python+to+Power+BI;Data+is+the+new+oil+—+We+refine+it!"/></p>
<p style="font-size:24px; line-height:1.8;">
Kerolles Abo alnasr Kamal<br>
Ibrahim Mohamed Ismail<br>
Yousef Moustafa Mohamed<br>
Ibrahim Mohamed Ismail<br>
Mohamed Abd ellatif Abd elmonem<br>
Mohamed Mahmoud Hosni
</p>  



---

<h1>Project Overview</h1>
<p>The <strong>HR Attrition Analysis</strong> project is a cross-functional data analytics initiative designed to uncover why employees leave companies. Using simulated data from a fictional tech firm, we walk through the entire data pipeline:</p>
<ul>
  <li>✅ <strong>Data Cleaning</strong></li>
  <li>✅ <strong>Data Modeling</strong></li>
  <li>✅ <strong>Visualization</strong></li>
  <li>✅ <strong>Business Recommendations</strong></li>
</ul>


---

Tools & Technologies

<p>
  <img alt="Python" src="https://img.shields.io/badge/-Python-3776AB?style=for-the-badge&logo=python&logoColor=white"/>
  <img alt="Pandas" src="https://img.shields.io/badge/-pandas-150458?style=for-the-badge&logo=pandas&logoColor=white"/>
  <img alt="SQL" src="https://img.shields.io/badge/-SQL-0064a5?style=for-the-badge&logo=mysql&logoColor=white"/>
  <img alt="Excel" src="https://img.shields.io/badge/-Excel-217346?style=for-the-badge&logo=microsoft-excel&logoColor=white"/>
  <img alt="PowerBI" src="https://img.shields.io/badge/-Power%20BI-F2C811?style=for-the-badge&logo=microsoft-power-bi&logoColor=black"/>
  <img alt="Tableau" src="https://img.shields.io/badge/-Tableau-005CAA?style=for-the-badge&logo=tableau&logoColor=white"/>
  <img alt="Git" src="https://img.shields.io/badge/-Git-F05032?style=for-the-badge&logo=git&logoColor=white"/>
</p>
<p>

---
<h1>🧪 Methodology</h1>
<ul style="font-size:18px; line-height:1.8;">
  <li><strong>Data Cleaning</strong> → Checked for missing values, incorrect types, duplicates</li>
  <li><strong>Data Modeling</strong> → Built a star schema</li>
  <li><strong>Exploratory Data Analysis (EDA)</strong> → Used Python & Excel to uncover trends, develop hypotheses</li>
  <li><strong>Dashboards</strong> → Created actionable visualizations with Power BI & Tableau</li>
</ul>
---  

<h1>How to Use</h1>
<div>
  <h3>• Explore Excel Files</h3>
  <p>Open <code>.xlsx</code> files for raw data and pivot tables.</p>

  <h3>• Run Python Notebooks</h3>
  <p>Ensure you have installed the required packages:</p>
  <pre><code>pip install pandas matplotlib seaborn</code></pre>

  <h3>• Open Dashboards</h3>
  <p>Power BI → <code>.pbix</code> files  <br>Tableau → <code>.twbx</code> or <code>.twb</code> files</p>

  <p>⚠ <strong>Important:</strong> Some file paths may need adjustment depending on your local setup.</p>
</div>


---

<h1>Project Link</h1>
- [Google Drive Folder](https://drive.google.com/drive/folders/1nJA8oBAAJ-Ir63alAvJKUeM3un5LP371?usp=drive_link)

-<h1>LinkedIn Profiles</h1>
- [Kerolles Abo alnasr](https://www.linkedin.com/in/kerolles-abo-alnasr)
- [Ibrahim Mohamed Ismail](https://www.linkedin.com/in/ibrahimnoureldin0)
- [Mohamed Abd ellatif Abd elmonem](https://www.linkedin.com/in/mohammed-abd-ellatif-13158b29a)
- [Mohamed Mahmoud Hosni](https://www.linkedin.com/in/mohamed-hosni-13a23129a)
- [Yousef Moustafa Mohamed](https://www.linkedin.com/in/yousef-moustafa-a3b48731a)
- [Kareem Mohamed](https://www.linkedin.com/in/kareem-mohamed-2b05b8180)
---
